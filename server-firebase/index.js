...
app.post("/api/snapshot", basicAuth, async (req, res) => {
  ...
  try {
    ...
    for (const relay of relays) {
      ...
      if (!key) continue;
      const deletedAt = deletedRelayByKey.get(key) || 0;
      const incomingSync = relaySyncValue(relay);
      if (deletedAt > 0 && incomingSync <= deletedAt) {
        // Ignore stale payloads that try to resurrect a deleted relay.
        continue;
      }
      if (deletedAt > 0 && incomingSync > deletedAt) {
        // A newer payload can legitimately recreate the relay.
        await db.run(
          "DELETE FROM relay_deletions WHERE userId = ? AND relayKey = ?",
          userId,
          key
        );
        deletedRelayByKey.delete(key);
      }
